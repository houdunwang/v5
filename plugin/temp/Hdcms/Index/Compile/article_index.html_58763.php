...
<script src='http://localhost/v5/plugin/hd/HDPHP/hdphp/Extend/Org/bootstrap/js/bootstrap.min.js'></script>
                ...
                <![endif]-->
    <link rel="stylesheet"  type='text/css' href="http://localhost/v5/plugin/template/default/css/article_list.css"/>
    <!--[if lt IE 8]>
...
<div class="location">
	<div class="nums">
		主题数：<?php echo $hdcms['article_num'];?> &nbsp;&nbsp;&nbsp;回复数：<?php echo $hdcms['comment_num'];?>
		<a href="http://localhost/v5/plugin/index.php?a=Member&c=Content&m=selectCategory&mid=<?php echo $hdcms['mid'];?>&cid=<?php echo $hdcms['cid'];?>" class="publish">
                <strong>+</strong>发表文章
            </a>
	</div>
	<h3>        <?php
    ...
    <div class="row">
        ...
        if($result):
            ...
            ?>
	                    <div class="article">
	                    	<div class="icon">
	                    		<img src="<?php echo $field['icon'];?>" alt="<?php echo $field['username'];?>" onmouseover="user.show(this,<?php echo $field['uid'];?>)"/>
	                    	</div>
	                    	<div class="title">
	                          	<a href="<?php echo $field['url'];?>" target="_blank"><?php echo $field['title'];?></a>
							</div>
							<div class="info">
								<span class="ot-list-time pull-right"><?php echo date('Y-m-d H:i',$field['addtime']);?></span>
								<a href="/cate/site.html"><?php echo $field['catname'];?></a>|
								<a class="ot-list-author" href="/u/3.html"><?php echo $field['nickname'];?></a>
								<span>浏览：<?php echo $field['click'];?> 回复：<?php echo $field['comment_num'];?></span>	
							</div>
	                    </div>
        ...
        ?>
            ...
            foreach($data as $field):
                ...
                $field['icon']=$field['icon']?'http://localhost/v5/plugin/'.$field['icon']:'http://localhost/v5/plugin/data/image/user/150.png';
            ?>
            ...
            foreach($data as $field):
                ...
                $field['userlink'] = ' http://localhost/v5/plugin/index.php?' . $_tmp;
                $field['url']='http://localhost/v5/plugin/index.php?a=Index&c=Index&m=content&mid='.$field['mid'].'&cid='.$field['cid'].'&aid='.$field['aid'].'&comment_id='.$field['comment_id'];
                $field['content'] =mb_substr($field['content'],0,20,'utf-8');
                $field['pubtime'] =date_before($field['pubtime']);
                $field['icon']=$field['icon']?'http://localhost/v5/plugin/'.$field['icon']:'http://localhost/v5/plugin/data/image/user/100.png';
            ?>
                    <section>
                        <a href="<?php echo $field['url'];?>">
