    ...
    <div id="main">
        ...
        <div class="content">
        	<style type="text/css">
        		...
        		table,td{
        			border:solid 1px #f3f3f3;padding:5px 10px;
        		}
        		div.tplPic{
        		...
        		div.tplPic img{
        			border:solid 1px #f3f3f3;padding:5px;
        			max-width: 500px;
        		}
        	</style>
        	<h4>作者信息：</h4>
        	<table>
        		<tr>
        			...
        			<td><?php echo _default($hdcms['qq'],'保密');?></td>
        		</tr>
        		<tr>
        			<td>邮箱：</td>
        			<td><?php echo _default($hdcms['email'],'保密');?></td>
        		</tr>
        		<tr>
        			<td>电话：</td>
        			<td><?php echo _default($hdcms['tel'],'保密');?></td>
        		</tr>
        		<tr>
        			<td>作者介绍：</td>
        			<td><?php echo _default($hdcms['about'],'这家伙很懒，没有写介绍');?></td>
        		</tr>
        	</table>
        	<hr />
        	<h4>模板预览：</h4>
        		<?php $hd["list"]["pic"]["total"]=0;if(isset($hdcms['pics']['path']) && !empty($hdcms['pics']['path'])):$_id_pic=0;$_index_pic=0;$lastpic=min(1000,count($hdcms['pics']['path']));
$hd["list"]["pic"]["first"]=true;
$hd["list"]["pic"]["last"]=false;
$_total_pic=ceil($lastpic/1);$hd["list"]["pic"]["total"]=$_total_pic;
$_data_pic = array_slice($hdcms['pics']['path'],0,$lastpic);
if(count($_data_pic)==0):echo "";
else:
foreach($_data_pic as $key=>$pic):
if(($_id_pic)%1==0):$_id_pic++;else:$_id_pic++;continue;endif;
$hd["list"]["pic"]["index"]=++$_index_pic;
if($_index_pic>=$_total_pic):$hd["list"]["pic"]["last"]=true;endif;?>

        			<div class="tplPic">
        			<img src="http://localhost/v5/plugin/<?php echo $pic;?>" />
        			</div>
        		<?php $hd["list"]["pic"]["first"]=false;
endforeach;
endif;
else:
echo "";
        	...
        	<h4>文件下载：</h4>
        	<?php if(is_array($hdcms['tplfile']['path'])):?><?php  foreach($hdcms['tplfile']['path'] as $key=>$value){ ?>
        		文件<?php echo $key+1;?>: <a href="http://localhost/v5/plugin/<?php echo $value;?>"><?php echo $hdcms['tplfile']['alt'][$key];?></a>
        	<?php }?><?php endif;?>
        </div>
        <div class="tag">
            <?php if($hdcms['tag']){?>
